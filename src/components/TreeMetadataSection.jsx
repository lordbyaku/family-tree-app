...
const TreeMetadataSection = () => {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        const res = await updateTreeMetadata(formData);
        if (res.success) {
            success("Informasi silsilah berhasil diperbarui!");
            setIsEditing(false);
        } else {
            toastError("Gagal memperbarui informasi.");
        }
        setIsSaving(false);
    };
    ...
    return (
        <div className="mb-6 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
                        ...
                        <div className="flex-1">
                            {isEditing ? (
                                ...
                                <h1 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white tracking-tight flex items-center gap-3">
                                    <BookOpen className="text-blue-600 dark:text-blue-400 shrink-0" size={28} />
                                    {treeMetadata.title}
                                </h1>
                            )}
                        </div>
                ...
                <div className="px-6 py-3 bg-slate-50 dark:bg-slate-900/50 flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                    <span className="flex items-center gap-1.5">
                        <Info size={12} />
                        Mode: <span className="text-blue-600 dark:text-blue-400">{treeSlug === 'gabungan' ? 'Gabungan' : 'Eksklusif'}</span>
                    </span>
                </div>
            </div>
        </div>
